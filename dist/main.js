...
function handleVolunteer($s) {
  ...
  $s.submit((e) => {
    ...
    const name = $s.find("[name=name]").val();
    const email = $s.find("[name=email]").val();
    const bg = $s.find("[name=background]").val();
...
function handleSmoothScroll() {
  const menuHeight = $("#visible-step").length ? 70 : 50;
...
$(function() {
  ...
  $("#volunteer").length && handleVolunteer($("#volunteer"));
  !$("#stories").length && handleSmoothScroll();
});
